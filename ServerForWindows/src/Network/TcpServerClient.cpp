...
DataBuffer TcpServer::Client::loadData()
{
    ...
    int answ = recv(socket, &size, sizeof(size), NIX(MSG_DONTWAIT)WIN(0));
    ...
    while ( ( data_read = recv ( socket, &data, sizeof ( data ), 0 ) ) > 0 && data != '\n' )
    {
...
//     DataBuffer data;
//     char size;
//     int answ = recv ( socket, &size, sizeof ( size ), MSG_DONTWAIT );
//     
//     if(!size) return DataBuffer();
//     data.push_back ( size );
//     
//     while ( ( answ = recv(socket, &size, sizeof ( size ), MSG_DONTWAIT ) ) > 0  && size != '\n' )
//     {
//         if (answ == -1) {
//             perror("CLIENT: ERROR recv()");
//             exit(EXIT_FAILURE);
